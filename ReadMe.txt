...
  - Validates configuration BEFORE the scan (fail-fast pre-flight check)
...
================================================================================
 
  checkmarx_scan.py                       Main Python script (scan + email)
  validate_config.py                      Pre-flight config validator (Python)
...
      * API Key (created via the Checkmarx UI)
      * Tenant name
      * (Optional) A group you are a member of
 
...
 
            CX_BASE_URI       = https://deu.ast.checkmarx.net
            CX_TENANT         = cx-cs-na-pspoc
            CX_APIKEY         = <your Checkmarx API key>
            CX_GROUP          = <your group name, or leave unset>
 
...
 
          The workflow runs the FULL pipeline on all three OSes to prove
          cross-platform compatibility. However, only the Ubuntu run sends
          the actual email - Windows and macOS use --skip-email to avoid
          duplicate notifications while still validating the script works.
 
          Each run:
            - Installs Python and the cx CLI
            - Runs validate_config.py (pre-flight check)
            - Runs the scan
            - Sends the email (Ubuntu only)
...
 
          Catches misconfig in <30 seconds before any scan runs.
 
...
  modularity the requirement enables.
 
 
================================================================================
...
                        (e.g. https://deu.ast.checkmarx.net)
...
                        If empty, the project will be created without a group.
...
    OUTPUT_NAME         Report filename without extension
 
 
================================================================================
...
  compatibility. To avoid sending 3 duplicate emails per commit, only
  the Ubuntu run sends the email - the Windows and macOS runs use
  --skip-email but still execute the full scan + parse pipeline.
 
 
================================================================================
10. NOTE ON ASSIGNMENT TASK 2 vs TASK 3b
================================================================================
 
  Tasks 2 and 3b both reference creating an API key:
    - Task 2:  "Create an APIKEY in the platform to be used with the CLI"
    - Task 3b: "Create an APIKEY in the platform to be used with the CLI"
 
  Since Checkmarx One API keys are created via the UI (Settings > Identity
  and Access Management > API Keys) by a human with appropriate permissions,
  and there is no public CLI command to programmatically generate API keys,
  both tasks were interpreted as the same action: creating one API key
  manually in the Checkmarx One UI.
 
  The API key is stored in GitHub Secrets as CX_APIKEY and consumed by the
  CLI via the --apikey flag.
 
 
================================================================================
11. NOTE ON CHECKMARX EMAIL FEEDBACK APP (Alternative approach)
================================================================================
 
...
 
  Problem: Pre-flight validator says "Authentication failed"
  Fix:     Verify CX_APIKEY, CX_TENANT, and CX_BASE_URI in your Secrets.
           Test locally:
             cx auth validate --apikey X --base-uri Y --tenant Z
 
...
  Checkmarx One DEU:   https://deu.ast.checkmarx.net
